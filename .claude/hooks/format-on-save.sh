...
# clang-format (C, C++, Objective-C). Config: .clang-format at project root.
if [ "$FORMATTED" = false ] && command -v clang-format >/dev/null 2>&1 && [ -f "$ROOT/.clang-format" ]; then
  case "$EXTENSION" in
    c|h|cc|cpp|cxx|hpp|hxx|inc|mm|m)
      clang-format --style=file -i "$FILE_PATH" >/dev/null 2>&1 && FORMATTED=true
      ;;
  esac
fi

# cmake-format (CMake files). Config: .cmake-format.yaml at project root.
if [ "$FORMATTED" = false ] && command -v cmake-format >/dev/null 2>&1 && [ -f "$ROOT/.cmake-format.yaml" ]; then
  case "$EXTENSION" in
    cmake)
      cmake-format -i "$FILE_PATH" >/dev/null 2>&1 && FORMATTED=true
      ;;
  esac
  # CMakeLists.txt has no extension; match by basename.
